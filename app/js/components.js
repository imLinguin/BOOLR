...
function add(
  ...
  if ((!findPortByPos(x, y) && !findWireByPos(x, y)) || force) {
    ...
    if (
      undoable &&
      component.constructor == Custom &&
      component.input.length + component.output.length == 0
    ) {
      component.open();
    }
...
function removeSelection(components, wires, undoable = false) {
  ...
  if (undoable) {
    ...
    undoStack.push(() => {
      addSelection(components, wires, selection, undefined, undefined);
...
function changeSize(
  ...
  for (let i = 0; i < ports.length; ++i) {
    const port = ports[i];
    if (
      (port.pos.side % 2 == 1 && port.pos.pos > height - 1) ||
      ...
      while (findPortByComponent(component, side, pos)) {
        ...
        if (pos < 0) {
          ...
          (side % 2 == 0 && pos > width - 1) ||
          (side % 2 == 1 && pos > height - 1)
        ) {
          side = ++side % 4;
          pos = 0;
        }
  ...
  for (let i = 0; i < component.input.length; ++i) {
    ...
    if (wire) {
      ...
      const index = wire.pos.findIndex(
        (pos, i) =>
          i < wire.pos.length - 1 &&
          pos.x == wire.pos.slice(-1)[0].x &&
          pos.y == wire.pos.slice(-1)[0].y
      );
      ...
      while (Math.abs(dx) + Math.abs(dy) > 0) {
        ...
        wire.pos.splice(wire.pos.length - 1, 0, {
    ...
    if (port.type == "input") {
      ...
      const vertical = () => {
        ...
        for (let i = 0; i < Math.abs(dy); ++i) {
          wire.pos.splice(wire.pos.length - 1, 0, { x, y });
        }
      };
...
function moveSelection(components, wires, dx, dy, undoable = false) {
  for (let i = 0; i < components.length; ++i) {
...
function edit(object, property, value, undoable = false) {
  if (
    ...
    typeof value == typeof object[property]
  ) {
...
function findPortByComponent(component, side, pos) {
  return (
    component.input.find(
      (port) => port.pos.side == side && port.pos.pos == pos
    ) ||
    component.output.find(
      (port) => port.pos.side == side && port.pos.pos == pos
    )
  );
}
...
function findPortByID(id) {
  for (let i = 0; i < components.length; ++i) {
    const found =
      components[i].input.find((port) => port.id == id) ||
...
function findWiresInSelection(
  ...
  for (let i = 0; i < wires.length; ++i) {
    const pos = wires[i].pos;
    for (let j = 0; j < pos.length; ++j) {
      if (pos[j].x >= x && pos[j].x <= x2 && pos[j].y >= y && pos[j].y <= y2) {
        result.push(wires[i]);
        break;
      }
    }
...
function cloneComponent(component, dx = 0, dy = 0) {
  ...
  if (clone.name.includes(clone.constructor.name + "#")) {
    clone.name =
      ...
      "#" +
      components.filter((a) => a.constructor == clone.constructor).length;
  }
...
function cloneSelection(components = [], wires = [], dx = 0, dy = 0) {
  ...
  for (let i = 0; i < wires.length; ++i) {
    ...
    if (
      (wire.from && !components.includes(wire.from.component)) ||
    ...
    const fromPort =
      clonedComponents[fromIndex] &&
      clonedComponents[fromIndex].output[fromPortIndex];
    const toPort =
      clonedComponents[toIndex] && clonedComponents[toIndex].input[toPortIndex];
  ...
  for (let i = 0; i < wires.length; ++i) {
    for (let j = 0; j < wires[i].input.length; ++j) {
      ...
      if (index > -1) {
        connectWires(clonedWires[index], clonedWires[i]);
      }
    }
    ...
    for (let j = 0; j < wires[i].output.length; ++j) {
      ...
      if (index > -1) {
        connectWires(clonedWires[i], clonedWires[index]);
      }
    }
  }
...
function componentize(
  ...
  if (undoable) {
    ...
    undoStack.push(() => {
      ...
      addSelection(removed.components, removed.wires, selection);
...
class DLATCH extends Component {
  constructor(name, pos) {
    super(name, pos, 4, 2, { type: "char", text: "D-LATCH" });
    this.addInputPort({ side: 3, pos: 0 }, "DATA");
    this.addInputPort({ side: 3, pos: 1 }, "CLOCK");
    this.addOutputPort({ side: 1, pos: 0 });
  }

  function() {
    if (this.input[1].value == 1) {
...
class SRLATCH extends Component {
  constructor(name, pos) {
    super(name, pos, 4, 2, { type: "char", text: "SR-LATCH" });
    this.addInputPort({ side: 3, pos: 0 }, "SET");
    this.addInputPort({ side: 3, pos: 1 }, "RESET");
...
class Custom extends Component {
  constructor(name, pos, components = [], wires = [], description = "") {
    super(name, pos);
